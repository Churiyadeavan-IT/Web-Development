...
/* Displays alert message if no vehicle is selected */
function alert_unselected() {
    let arr = []
    const allEqual = arr => arr.every(v => v === 'false')
    container_outer.forEach(function (vehicle) {
        arr.push(vehicle.dataset.vehicleSelected)
    })
    if (allEqual(arr)) {
        alert("Select any one vehicle or multiple vehicles to count it")
    }
}
